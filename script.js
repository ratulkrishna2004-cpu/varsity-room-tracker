...
function init() {
    ...
    grid.innerHTML = "";
    periods.forEach(p => {
        grid.innerHTML += `<label class="bg-white/80 p-2 rounded-lg flex items-center gap-1 cursor-pointer text-[8px] text-indigo-950 font-black hover:bg-white transition"><input type="checkbox" value="${p}" class="p-check"> ${p}</label>`;
    });
...
window.loginMentor = function() {
    ...
    if (found) {
        ...
        document.getElementById('authArea').innerHTML = `<span class="bg-green-600 text-white px-3 py-2 rounded-lg text-[10px] font-black uppercase">Verified</span>`;
...
window.searchRooms = function() {
    ...
    const slots = Array.from(document.querySelectorAll('.p-check:checked')).map(c => c.value);
    if (slots.length === 0) return;

    roomData.filter(r => r.day.toLowerCase() === day.toLowerCase()).forEach(r => {
        const matched = slots.filter(s => r.slots.includes(s));
        if (matched.length === 0) return;
        const busy = liveBookings.some(b => b.room === r.room && b.date === date);

        div.innerHTML += `
            <div class="bg-white/95 p-6 rounded-[32px] shadow-2xl relative">
                ${busy ? `<span class="absolute top-0 right-0 bg-red-600 text-white text-[8px] font-black px-3 py-1 rounded-bl-xl">BOOKED</span>` : ''}
                <h3 class="text-2xl font-black text-indigo-950 mb-3 italic uppercase">${r.room}</h3>
                ...
                ${currentMentor && !busy ? `<button onclick="openBooking('${r.room}', '${day}')" class="w-full bg-indigo-600 text-white py-2 rounded-xl text-[10px] font-black uppercase shadow-lg">Book Now</button>` : ''}
            </div>`;
    });
};
...
window.openBooking = function(room, day) {
    ...
    document.getElementById('bookSlotList').innerHTML = currentRoom.slots.map(s => `<label class="flex items-center gap-3 p-4 bg-white/5 rounded-2xl cursor-pointer hover:bg-white/10 mb-2 border border-white/5"><input type="checkbox" value="${s}" class="book-check w-5 h-5 accent-indigo-500"> <span class="text-xs font-black text-white">${s}</span></label>`).join('');
...
window.submitBooking = function() {
    ...
    alert("Room Booked!"); closeModal('bookingModal');
};
...
function updateLiveUI() {
    ...
    liveBookings.sort((a,b) => new Date(a.date) - new Date(b.date)).forEach(b => {
        l.innerHTML += `
            <div class="p-5 bg-white/5 rounded-[24px] border border-indigo-500/20 mb-4 text-left">
                ...
                <p class="text-[10px] text-white font-bold italic mb-3">${b.slot}</p>
                <div class="flex justify-between items-center pt-3 border-t border-white/5">
                    <span class="text-[9px] bg-indigo-500 text-white px-2 py-0.5 rounded font-black uppercase">${b.wing}</span>
                    <span class="text-[9px] text-green-400 font-black italic">By: ${b.mentor}</span>
                </div>
            </div>`;
    });
}
