...
function init() {
    ...
    if(daySelect) daySelect.value = days[now.getDay()] || "Sunday";
    
    const dateSelect = document.getElementById('dateSelect');
    if(dateSelect) dateSelect.value = now.toISOString().split('T')[0];
...
window.loginMentor = function() {
    ...
    if (found) {
        ...
        document.getElementById('authArea').innerHTML = `<span class="bg-green-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-tighter shadow-lg shadow-green-900/20">Mentor: ${found.name.split(' ')[0]}</span>`;
        closeModal('mentorModal');
        searchRooms();
    } else { alert("Access Denied: Invalid Mentor Code"); }
};
...
async function loadCSV() {
    ...
    } catch(e) { console.error("Routine Load Error"); }
}
    ...
    roomData.filter(r => r.day.toLowerCase() === day.toLowerCase()).forEach(r => {
        ...
        div.innerHTML += `
            <div class="bg-white/95 p-8 rounded-[40px] shadow-2xl relative transition-all hover:-translate-y-1">
                ${busy ? `<span class="absolute top-0 right-0 bg-red-600 text-white text-[9px] font-black px-4 py-1.5 rounded-bl-2xl uppercase tracking-tighter animate-pulse">Occupied</span>` : ''}
                ...
                ${currentMentor && !busy ? `<button onclick="openBooking('${r.room}', '${day}')" class="w-full bg-indigo-600 text-white py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition-all active:scale-95">Book This Room</button>` : ''}
            </div>`;
    });
};
...
window.openBooking = function(room, day) {
    ...
    document.getElementById('bookSlotList').innerHTML = currentRoom.slots.map(s => `
        <label class="flex items-center gap-4 p-5 bg-white/5 rounded-[24px] cursor-pointer hover:bg-white/10 mb-2 border border-white/5 transition-all">
            <input type="checkbox" value="${s}" class="book-check w-5 h-5 accent-indigo-500 shadow-lg"> 
...
window.submitBooking = function() {
    ...
    if (!wing || selected.length === 0) return alert("Validation Error: Choose Wing and Slots");
    ...
    alert("Success! Room " + currentRoom.room + " is now registered.");
...
function updateLiveUI() {
    ...
    if(liveBookings.length === 0) {
        l.innerHTML = `<div class="py-20 text-center opacity-20"><p class="text-[10px] font-black uppercase tracking-[0.3em]">No Classes Found</p></div>`;
        return;
    }
    liveBookings.sort((a,b) => new Date(a.date) - new Date(b.date)).forEach(b => {
        l.innerHTML += `
            <div class="p-6 bg-white/5 rounded-[32px] border border-white/10 mb-4 shadow-inner">
                <div class="flex justify-between items-start mb-3"><p class="font-black text-indigo-400 text-xl tracking-tighter italic">${b.room}</p><span class="text-[9px] bg-white/10 px-3 py-1 rounded-xl text-white font-black">${b.date}</span></div>
                <p class="text-[11px] text-white/70 font-bold mb-4 italic tracking-tight">${b.slot}</p>
                <div class="flex justify-between items-center pt-4 border-t border-white/5">
                    <span class="text-[9px] bg-indigo-600 text-white px-3 py-1 rounded-lg font-black uppercase tracking-widest">${b.wing}</span>
                    <span class="text-[9px] text-green-400 font-black italic tracking-tighter uppercase opacity-80">By: ${b.mentor.split(' ')[0]}</span>
                </div>
            </div>`;
    });
}
